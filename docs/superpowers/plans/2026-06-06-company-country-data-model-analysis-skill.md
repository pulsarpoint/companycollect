# Company Country Data Model Analysis Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a Codex skill that turns a completed country open-data investigation into per-source field catalogs and a country-specific company JSON profile proposal.

**Architecture:** Create one new focused skill under `companycollect/skills/company-country-data-model-analysis/`. The skill consumes existing `companycollect/companies/{country_slug}` investigation outputs and writes all analysis artifacts under that country's `analysis/` directory. The skill remains documentation-only; validation is a contract check over the required trigger metadata, workflow, output files, and per-source catalog schema.

**Tech Stack:** Markdown skill instructions, YAML frontmatter, shell/Python validation commands, git.

---

## File Structure

- Create: `companycollect/skills/company-country-data-model-analysis/SKILL.md`
  - Defines when the skill triggers, input contract, output directory layout, per-source `source_field_catalog.json` schema, country-specific combined profile rules, mapping reports, safety rules, and final response format.
- No production code files are needed.
- No generated country analysis files are part of this implementation. Running the skill for Serbia/Germany/etc. is a separate use of the skill.
...
path = Path("skills/company-country-data-model-analysis/SKILL.md")
required = [
...
if not path.exists():
    raise SystemExit("missing skill file")

text = path.read_text()
missing = [item for item in required if item not in text]
if missing:
    raise SystemExit("missing required text: " + ", ".join(missing))

print("skill contract check passed")
PY
```

Expected: FAIL with `missing skill file`.
...
```bash
cd /Users/graovic/pulsarpoint/ppoint/companycollect
mkdir -p skills/company-country-data-model-analysis
```
...
Create `companycollect/skills/company-country-data-model-analysis/SKILL.md` with this content:
...
- Produce one `source_field_catalog.json` per public/open source.
...
Expected: PASS with `skill contract check passed`.
...
```bash
cd /Users/graovic/pulsarpoint/ppoint/companycollect
git add skills/company-country-data-model-analysis/SKILL.md
git commit -m "feat: add company country data model analysis skill"
```

Expected: commit succeeds and includes only the new skill file.

## Task 2: Validate The Skill Instructions

**Files:**
- Read: `companycollect/skills/company-country-data-model-analysis/SKILL.md`
- Read: `companycollect/docs/superpowers/specs/2026-06-06-company-country-data-model-analysis-skill-design.md`

- [ ] **Step 1: Run the red-flag wording scan**
...
text = Path("skills/company-country-data-model-analysis/SKILL.md").read_text()
patterns = [
    "TB" + "D",
    "TO" + "DO",
    "FIX" + "ME",
...
if matches:
    raise SystemExit("red-flag wording found: " + ", ".join(matches))
print("red-flag wording scan passed")
PY
```

Expected: PASS with `red-flag wording scan passed`.

- [ ] **Step 2: Run the required-output scan**
...
missing = [item for item in required_outputs if item not in text]
if missing:
    raise SystemExit("missing output contract: " + ", ".join(missing))
print("required output scan passed")
PY
```

Expected: PASS with `required output scan passed`.

- [ ] **Step 3: Run the country-specific guardrail scan**
...
text = Path("skills/company-country-data-model-analysis/SKILL.md").read_text().lower()
required_phrases = [
    "do not force countries into one shared global schema",
    "country-specific combined json",
    "keep per-source catalogs separate",
    "suggestion for a future cross-country mapper",
]
missing = [phrase for phrase in required_phrases if phrase not in text]
if missing:
    raise SystemExit("missing country-specific guardrails: " + ", ".join(missing))
print("country-specific guardrail scan passed")
PY
```

Expected: PASS with `country-specific guardrail scan passed`.
...
spec = Path("docs/superpowers/specs/2026-06-06-company-country-data-model-analysis-skill-design.md").read_text()
skill = Path("skills/company-country-data-model-analysis/SKILL.md").read_text()
terms = [
    "source_field_catalog.json",
    "country_company_profile.schema.json",
    "country_company_profile.example.json",
...
missing_from_spec = [term for term in terms if term not in spec]
missing_from_skill = [term for term in terms if term not in skill]
if missing_from_spec:
    raise SystemExit("spec missing expected term: " + ", ".join(missing_from_spec))
if missing_from_skill:
    raise SystemExit("skill missing expected term: " + ", ".join(missing_from_skill))
print("spec-to-skill coverage scan passed")
PY
```

Expected: PASS with `spec-to-skill coverage scan passed`.
...
cd /Users/graovic/pulsarpoint/ppoint/companycollect
git add skills/company-country-data-model-analysis/SKILL.md
git commit -m "docs: tighten company data model analysis skill"
```

Expected: commit succeeds only if validation edits were made. If no edits were made, skip this step.

## Self-Review

Spec coverage:

- Per-source field catalogs: Task 1 defines `analysis/sources/{source_slug}/source_field_catalog.json` and `source_field_catalog.md`.
- Country-specific combined profile: Task 1 defines schema/example outputs and explicitly rejects forced global schemas.
- Mapping report: Task 1 defines `country_company_profile_mapping.md` with joins, precedence, freshness, and missing-data notes.
- Common mapper suggestions: Task 1 defines `common_field_mapping_suggestions.md` as non-binding.
- Safety rules: Task 1 includes bounded sampling, license uncertainty, no invented fields, and no access-control bypass.
- Validation: Task 2 checks red-flag wording, required outputs, country-specific guardrails, and spec-to-skill terms.

Red-flag wording scan:

- No incomplete-marker, deferred-work, or copy-forward instructions are present.

Type/name consistency:

- Output file names match the approved spec.
- Per-source catalog fields consistently use `source_field`, `english_name`, `meaning`, `data_type`, `semantic_type`, `example_values`, `nullable`, `repeatable`, `code_list`, `language`, `limitations`, `source_confidence`, and `analysis_notes`.
