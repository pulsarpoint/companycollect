...
func copyFixtureSnapshot(t *testing.T, path string) {
	t.Helper()
	data, err := os.ReadFile(filepath.Join("testdata", "eo_bmf_sample.ndjson"))
	if err != nil {
...
func TestProcessStreamsChunksSkipsBadLineAndUsesLatest(t *testing.T) {
	...
	writeTestFile(t, olderPath, []byte(`{"EIN":"999999999","NAME":"OLD"}`+"\n"))
	...
	// 4 lines seen (3 valid + 1 malformed), 3 processed, 1 decode error.
