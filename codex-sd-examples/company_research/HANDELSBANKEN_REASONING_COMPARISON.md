...
To reproduce on the saved dataset, use a new output directory from the repository's `codex-sd-examples` directory:

```sh
.venv/bin/python company_research/benchmarks/compare_reasoning.py --part jobs --output company_research/data/handelsbanken-reasoning-repeat
.venv/bin/python company_research/benchmarks/compare_reasoning.py --part decisions --output company_research/data/handelsbanken-reasoning-repeat
.venv/bin/python company_research/benchmarks/compare_reasoning.py --part links --output company_research/data/handelsbanken-reasoning-repeat
.venv/bin/python company_research/benchmarks/audit_reasoning.py company_research/data/handelsbanken-reasoning-repeat
```

The harness reads only `DEEPSEEK` from `jobs_extraction_lab/.env`. It does not save credentials in the output.
