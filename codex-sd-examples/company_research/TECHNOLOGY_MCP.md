...
Install the optional MCP dependency from `codex-sd-examples`:

```sh
uv pip install --python .venv/bin/python -e './company_research[mcp]'
...
.venv/bin/company-technology-mcp \
  --offline-catalog \
  --technology-catalog company_research/tests/fixtures/technology_catalog.json
```
