...
async def run_jobs(root: Path, data: Path, key: str, effort: Literal["low", "high"]) -> None:
...
async def main(args: argparse.Namespace) -> None:
    ...
    if args.part == "jobs":
        ...
        shutil.copytree(
            Path("company_research/src/company_research"),
            root / "implementation",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data", type=Path, default=Path("company_research/data"))
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("company_research/data/handelsbanken-20260916-reasoning"),
    )
    parser.add_argument(
