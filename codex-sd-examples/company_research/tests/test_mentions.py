"""Evidence preservation, two-stage HTTP contract, catalog and immutable uploads."""
...
class UploadTests(unittest.TestCase):
    def test_conditional_create_identical_retry_and_collision(self):
        client = boto3.client(
            "s3",
        ...
        with TemporaryDirectory() as temporary, Stubber(client) as stub:
            ...
            params = {
                "Bucket": "crawls",
                "Key": "company-research/run/revision/result.json",
                "Body": payload,
                "ContentType": "application/json",
                "Metadata": ANY,
                "IfNoneMatch": "*",
            }
            get_params = {"Bucket": "crawls", "Key": params["Key"]}
            ...
            self.assertEqual(
                s3_results.upload_result(path, client, "crawls")["disposition"],
                "created",
            )
            ...
            stub.add_response(
                "get_object",
                {"Body": StreamingBody(io.BytesIO(b"changed"), 7)},
                get_params,
            )
            with self.assertRaisesRegex(ValueError, "collision"):
                s3_results.upload_result(path, client, "crawls")
