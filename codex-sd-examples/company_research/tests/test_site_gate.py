...
from company_research.models import SiteClassification
from company_research.research import ResearchConfig, research_company
...
class SiteGateTests(unittest.IsolatedAsyncioTestCase):
    async def run_gate(self, replies, *, fetched=True, calls=2, html=None):
        ...
        with (
            ...
            patch("company_research.research.open_browser", browser),
            patch(
                "company_research.research.fetch_page", side_effect=fetch
            ) as fetch_mock,
            patch.object(httpx.AsyncClient, "send", model_http),
            patch(
                "company_research.research.sitemap_urls", side_effect=sitemap
            ) as sitemap_mock,
            patch(
                "company_research.research.extract_saved_page", side_effect=extract
            ) as extraction,
            patch(
                "company_research.research.assess_links", new_callable=AsyncMock
            ) as ranking,
            patch(
                "company_research.research.summarize_company", return_value=None
            ) as summary,
            patch(
                "company_research.research.assess_external_links", return_value=None
            ) as external,
        ):
            root = Path(directory)
    ...
    async def test_schema_conflict_never_allows_a_content_site_to_continue(self):
        with self.assertLogs("company_research.research", level="ERROR"):
            result, _, calls = await self.run_gate(
                [
                    classification(
