...
@pytest.mark.asyncio
async def test_brreg_term_translation_maps_terms_through_llm_request() -> None:
    ...
    assert response.source_lang == "no"
    ...
    assert llm_request.source_lang == "no"
    assert llm_request.target_lang == "en"
    assert [item.model_dump(mode="json") for item in llm_request.items] == [
        {"id": TERM_KEY_1, "category": "brreg_term", "text": "Aksjeselskap"}
    ]


@pytest.mark.asyncio
async def test_brreg_term_translation_maps_missing_llm_terms_to_retryable_failures() -> None:
...
@pytest.mark.asyncio
async def test_brreg_term_translation_returns_retryable_failures_for_total_llm_exception() -> None:
    service = TranslationService(
