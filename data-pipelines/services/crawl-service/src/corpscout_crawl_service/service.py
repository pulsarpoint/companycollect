...
class CrawlService:
    ...
    async def analyze_search_page(self, request: SearchAnalyzeRequest | dict) -> SearchAnalyzeResponse:
        ...
        try:
            ...
            return SearchAnalyzeResponse(
                status="failed",
                error=_service_error_from_exception(
        ...
        return SearchAnalyzeResponse(
            status="succeeded",
            candidates=_scored_links_from_llm_output(
                output,
    ...
    async def analyze_company_page(self, request: PageAnalyzeRequest | dict) -> PageAnalyzeResponse:
        ...
        return PageAnalyzeResponse(
            status="succeeded",
            analysis=_apply_known_site_overrides(
                _apply_context_scoring_rules(output),
                normalized_domain=normalize_domain(request.normalized_domain),
            ),
        )
        ...
        if response.status == "succeeded":
            try:
                ...
                response.llm_output = {
                    ...
                    "evidence": [],
                    "analysis_error": error.model_dump(),
                }
...
def _domain_response(
    *,
    ...
    if not domains and not related_sites:
        response_errors.extend(_site_analysis_errors(site_checks))
    status = "failed" if response_errors else "succeeded" if domains else "not_found"
...
def _site_analysis_errors(site_checks: list[SiteCheckResult]) -> list[ServiceError]:
    errors: list[ServiceError] = []
    for check in site_checks:
        output = check.crawl.llm_output
        if not isinstance(output, dict):
            continue
        raw_error = output.get("analysis_error")
        if not isinstance(raw_error, dict):
            continue
        try:
            errors.append(ServiceError.model_validate(raw_error))
        except Exception:
            errors.append(
                ServiceError(
                    code="site_analysis_failed",
                    message=check.reason or "Site analysis failed.",
                    category="transient_external",
                    retry_strategy="retry_with_backoff",
                    detail={"url": check.url, "domain": check.normalized_domain},
                )
            )
    return errors


def _service_error_from_exception(*, code: str, message: str, exc: Exception) -> ServiceError:
    exception_type = type(exc).__name__
    exception_message = str(exc).strip()
    display_error = exception_message or exception_type
    return ServiceError(
        code=code,
        message=f"{message} {display_error}",
        category="transient_external",
        retry_strategy="retry_with_backoff",
        detail={"exception_type": exception_type, "error": exception_message},
    )
