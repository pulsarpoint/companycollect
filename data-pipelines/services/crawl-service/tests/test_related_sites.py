...
@pytest.mark.asyncio
async def test_existing_website_analysis_failure_is_retryable_failure() -> None:
    fake_crawler = FakeCrawl4AiService(
        {
            "https://a-logopedene.no": Crawl4AiResponse(
                url="https://a-logopedene.no",
                final_url="https://a-logopedene.no",
                status="succeeded",
                markdown="# Velkommen til A-logopedene\nA-Logopedene Dronningens Gate 6 Oslo.",
                markdown_hash="site-hash",
                links=[],
                duration_ms=11,
            ),
    ...
    service = CrawlService(
        ...
        site_analyzer=FailingSiteAnalyzer(),
    )

    response = await service.discover_domains(
        {
            "company_name": "A-LOGOPEDENE AS",
            "country": "NO",
            "existing_website": "https://a-logopedene.no",
        }
    )

    assert response.status == "failed"
    assert response.best_domain is None
    assert response.errors[0].code == "site_analysis_failed"
    assert response.errors[0].category == "transient_external"
    assert response.errors[0].retry_strategy == "retry_with_backoff"
    assert "TimeoutError" in response.site_checks[0].reason
