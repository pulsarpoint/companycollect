...
//
// This is a vendored, hardened copy of github.com/iand/microdata v0.0.28 (public domain,
// Unlicense). Upstream readItem recurses without bound: an itemref cycle (A->B->A — it guards
// only direct self-reference) or pathologically deep DOM nesting overflows the goroutine stack,
// which is an unrecoverable fatal error that kills the whole process. Observed in production on
// real Common Crawl pages. This copy adds a recursion depth cap and an itemref cycle guard;
...
// maxReadItemDepth bounds readItem recursion. Real microdata trees are a few levels deep; the
// cap only bites on pathological DOM nesting or itemref chains, where dropping the remainder
// beats an unrecoverable stack overflow.
const maxReadItemDepth = 512
...
type Parser struct {
	...
	depth           int                 // current readItem recursion depth
	refPath         map[*html.Node]bool // itemref nodes on the current expansion path (cycle guard)
}
		...
		if itemrefs, exists := getAttr("itemref", node); exists {
			for _, itemref := range strings.Split(strings.TrimSpace(itemrefs), " ") {
				...
				if refnode, exists := p.identifiedNodes[itemref]; exists {
					// Skip a node already being expanded on this path: itemref graphs on real
					// pages contain cycles (A->B->A), and re-entering one recurses forever.
					if refnode != node && !p.refPath[refnode] {
						if p.refPath == nil {
							p.refPath = make(map[*html.Node]bool)
						}
						p.refPath[refnode] = true
						p.readItem(item, refnode)
						delete(p.refPath, refnode)
					}
				}
			}
		}
