...
func run(ctx context.Context, logger *slog.Logger, args []string) error {
	...
	logEstimate(logger, options, rangeplanner.ExactEstimate(allRecords))
	...
	for _, scope := range scopes {
		for _, policy := range policies {
			...
			logEstimate(logger, options, estimate)
		}
	}
	...
	if options.wholeWARCSizes {
		...
		logEstimate(logger, options, rangeplanner.Estimate{
...
func logEstimate(logger *slog.Logger, options options, estimate rangeplanner.Estimate) {
