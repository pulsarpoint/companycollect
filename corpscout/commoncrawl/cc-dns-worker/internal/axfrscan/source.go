...
serfunc fetchSourcePage(ctx context.Context, connection driver.Conn, cursor string, limit int) ([]sourceDomain, error) {
	...
	for rows.Next() {
		...
		for index := range names {
			scope, valid := resolve.ClassifyString(ips[index])
			if !valid {
				scope = resolve.ScopeReserved
			}
			domain.Endpoints = append(domain.Endpoints, model.NameserverEndpoint{
				Name: names[index], IP: ips[index], Scope: string(scope), Dialable: resolve.Dialable(scope),
			})
		}
