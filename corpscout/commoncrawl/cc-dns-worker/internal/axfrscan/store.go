...
func (store *axfrStore) commit(ctx context.Context, scanID string, job probeJob, outcome resolve.AXFROutcome) error {
	transaction, err := store.database.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
	...
	if outcome.Verdict == resolve.VerdictOpen {
		column = "probes_open"
	} else if outcome.Verdict == resolve.VerdictClosed {
...
func (store *axfrStore) populateReady(ctx context.Context, scanID string, domain *readyDomain) error {
	...
	for rows.Next() {
		var outcome resolve.AXFROutcome
		...
		outcome.Verdict = resolve.AXFRVerdict(verdict)
		outcome.Reason = resolve.AXFRReason(reason)
