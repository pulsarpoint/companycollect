...
func RunCycle(ctx context.Context, dbPath string, config Config) error {
	...
	prober := resolve.NewAXFRProber(probeScheduler, resolve.AXFRCaps{
...
func axfrWorkLoop(ctx context.Context, store *axfrStore, config Config, prober *resolve.AXFRProber) error {
	poolContext, stopPool := context.WithCancel(ctx)
	defer stopPool()
	jobs := make(chan probeJob, config.ClaimBatch)
	results := make(chan struct {
		job     probeJob
		outcome resolve.AXFROutcome
	}, config.ClaimBatch)
