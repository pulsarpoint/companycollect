...
SELECT
  ...
  source_company.id AS source_company_id,
  source_company.payload_hash AS source_payload_hash,
  source_company.updated_at AS source_synced_at,
  CASE
    WHEN source_company.id IS NULL THEN 'not_synced'
    WHEN NULLIF(source_company.payload_hash, '') IS NOT NULL
     AND NULLIF(rr.payload_hash, '') IS NOT NULL THEN
      CASE
        WHEN source_company.payload_hash = rr.payload_hash THEN 'synced'
        ELSE 'needs_update'
      END
    WHEN rr.last_seen_at <= source_company.updated_at THEN 'synced'
    ELSE 'needs_update'
  END AS sync_status,
  CASE
    WHEN source_company.id IS NULL THEN false
    WHEN NULLIF(source_company.payload_hash, '') IS NOT NULL
     AND NULLIF(rr.payload_hash, '') IS NOT NULL THEN source_company.payload_hash = rr.payload_hash
    ELSE rr.last_seen_at <= source_company.updated_at
  END::boolean AS synced,
  rr.last_seen_at AS updated_at
