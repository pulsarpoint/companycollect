"""Lock-step tests for the Norway + Finland + Wikidata canonical-pair derivation
assets.
...
defs/finland_ytj/contacts.py, defs/wikidata/contacts.py) must replay EXACTLY those
SELECTs at runtime, or the asset's output would silently drift from the backfilled
data. This file extracts each migration's two INSERT SELECT bodies and asserts the
module's build_contacts_select()/build_domains_select() output matches, modulo
whitespace.
"""
...
def test_wikidata_canonical_contacts_asset_registered_with_dep():
    ...
    assert {k.path[-1] for k in node.parent_keys} == {"wikidata_company_seed_clickhouse"}
...
def test_wikidata_canonical_contacts_asset_in_weekly_job():
    ...
    keys = {
        k.path[-1]
        for k in repo.get_job("wikidata_company_seed_weekly_job").asset_layer.executable_asset_keys
    }
    assert "wikidata_clickhouse_canonical_contacts" in keys
