...
def test_financial_fetches_iterator_logs_candidate_count_and_progress(tmp_path: Path) -> None:
    database_path = tmp_path / "norway.duckdb"
    _seed_entities(database_path)
    messages: list[str] = []
    client = FakeDltRequestsClient(
    ...
    assert messages == [
        "Prepared Norway Brreg financial fetch candidates: candidates=4",
        (
            "Fetched Norway Brreg financial statements: processed=2 total=4 "
            "statuses={'not_found': 1, 'server_error': 1}"
        ),
        (
            "Fetched Norway Brreg financial statements: processed=4 total=4 "
            "statuses={'not_found': 1, 'server_error': 1, 'success': 1, "
            "'network_error': 1}"
        ),
        (
            "Completed Norway Brreg financial statement fetches: processed=4 total=4 "
            "statuses={'not_found': 1, 'server_error': 1, 'success': 1, "
            "'network_error': 1}"
        ),
    ]
