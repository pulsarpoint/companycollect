...
def test_sweden_company_snapshot_downloads_missing_files_to_s3() -> None:
    ...
    manifest_key = manifest_object_key(retrieved_date="2026-07-03")
...
def test_sweden_company_snapshot_skips_existing_source_date_keys() -> None:
    ...
    manifest = json.loads(
        object_store.objects[
            (
                SWEDEN_COMPANY_RAW_BUCKET,
                manifest_object_key(retrieved_date="2026-07-03"),
            )
        ]
    )
...
def test_sweden_company_manifest_for_run_prefers_current_run() -> None:
    ...
    object_store.write_json(
        manifest_object_key(retrieved_date="2026-07-02"),
        json.dumps(
            {
                "run_id": "other-run",
    ...
    object_store.write_json(
        manifest_object_key(retrieved_date="2026-07-01"),
        json.dumps(
            {
                "run_id": "current-run",
