...
def test_export_brazil_fin_cvm_clickhouse_table_exports_one_table(
    ...
    rows = clickhouse.export_brazil_fin_cvm_clickhouse_table(
...
def test_export_brazil_fin_cvm_clickhouse_table_refuses_empty_duckdb_table(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from dagster_v3.defs.brazil_financial.cvm import clickhouse
    ...
    with pytest.raises(ValueError, match="Brazil CVM DFP DuckDB table is empty"):
        clickhouse.export_brazil_fin_cvm_clickhouse_table(
            duckdb_connection=FakeDuckDBConnection(
                counts={(BRAZIL_CVM_DUCKDB_SCHEMA, DFP_DOCUMENTS_TABLE): 0}
            ),
            clickhouse=FakeClickHouseResource(),
            duckdb_table=DFP_DOCUMENTS_TABLE,
            clickhouse_table=tables.BR_CVM_DFP_DOCUMENTS_TABLE,
