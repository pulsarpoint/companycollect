...
ITR_DOCUMENTS_TABLE = "itr_documents"
ITR_STATEMENT_ROWS_TABLE = "itr_statement_rows"
ITR_CAPITAL_COMPOSITION_TABLE = "itr_capital_composition"
ITR_AUDITOR_REPORTS_TABLE = "itr_auditor_reports"
ITR_PARSE_RUNS_TABLE = "itr_parse_runs"

SOURCE_SLUG = "brazil_cvm_itr"
CSV_ENCODING = "latin-1"
CSV_FALLBACK_ENCODING = "utf-8"
CSV_WINDOWS_1252_ENCODING = "cp1252"
ITR_STATEMENT_ROWS_COLUMNS = (
    "country_iso2",
    "source_slug",
    "source_run_id",
    "source_record_id",
...
class BrazilCvmItrStatementMember:
    file_name: str
    statement_code: str
    statement_name: str
...
def parse_itr_statement_member_name(
    file_name: str,
    *,
    year: str | int,
) -> BrazilCvmItrStatementMember:
    ...
    if match is None:
        raise ValueError(f"Not a Brazil CVM ITR statement CSV member: {file_name}")
    statement_code = match.group("statement")
    return BrazilCvmItrStatementMember(
        file_name=file_name,
...
def load_brazil_fin_cvm_itr_archive(
    *,
    connection: Any,
    ...
    with tempfile.TemporaryDirectory(prefix="brazil_fin_cvm_itr_csv_") as tmpdir:
...
def parse_brazil_fin_cvm_itr_archive_from_object_store(
    *,
    connection: Any,
    object_store: Any,
    year: str | int,
    ...
    with tempfile.TemporaryDirectory(prefix="brazil_fin_cvm_itr_archive_") as tmpdir:
        archive_path = Path(tmpdir) / f"itr_cia_aberta_{normalized_year}.zip"
        object_store.download_file(
            archive_key,
            archive_path,
            bucket=BRAZIL_CVM_RAW_BUCKET,
        )
        return load_brazil_fin_cvm_itr_archive(
            connection=connection,
...
class _ExtractedMembers:
    document: Path | None
    statements: tuple[tuple[BrazilCvmItrStatementMember, Path], ...]
...
def _extract_known_members(
    *,
    ...
    document_name = f"itr_cia_aberta_{year}.csv"
    capital_name = f"itr_cia_aberta_composicao_capital_{year}.csv"
    auditor_name = f"itr_cia_aberta_parecer_{year}.csv"
    document_path: Path | None = None
    capital_path: Path | None = None
    auditor_path: Path | None = None
    statements: list[tuple[BrazilCvmItrStatementMember, Path]] = []
    ...
    with ZipFile(archive_path) as zip_file:
        for member_name in zip_file.namelist():
            ...
            if "/" in member_name:
                raise ValueError(
                    f"Unexpected Brazil CVM ITR nested member path: {member_name}"
                )
            if not member_name.endswith(".csv"):
                raise ValueError(
                    f"Unexpected Brazil CVM ITR archive member: {member_name}"
                )
            ...
            if member_name == document_name:
                ...
                try:
                    statement = parse_itr_statement_member_name(member_name, year=year)
                except ValueError as exc:
                    raise ValueError(
                        f"Unexpected Brazil CVM ITR CSV member: {member_name}"
                    ) from exc
                target_path.write_bytes(zip_file.read(member_name))
...
def _ensure_tables(connection: Any) -> None:
    ...
    connection.execute(
        f"""
        create table if not exists {BRAZIL_CVM_DUCKDB_SCHEMA}.{ITR_DOCUMENTS_TABLE} (
    ...
    connection.execute(
        f"""
        create table if not exists {BRAZIL_CVM_DUCKDB_SCHEMA}.{ITR_STATEMENT_ROWS_TABLE} (
            country_iso2 varchar,
            source_slug varchar,
            source_run_id varchar,
            source_record_id varchar,
    ...
    connection.execute(
        f"""
        create table if not exists {BRAZIL_CVM_DUCKDB_SCHEMA}.{ITR_CAPITAL_COMPOSITION_TABLE} (
            country_iso2 varchar,
            source_slug varchar,
            source_run_id varchar,
            source_record_id varchar,
    ...
    connection.execute(
        f"""
        create table if not exists {BRAZIL_CVM_DUCKDB_SCHEMA}.{ITR_AUDITOR_REPORTS_TABLE} (
            country_iso2 varchar,
            source_slug varchar,
            source_run_id varchar,
            source_record_id varchar,
    ...
    connection.execute(
        f"""
        create table if not exists {BRAZIL_CVM_DUCKDB_SCHEMA}.{ITR_PARSE_RUNS_TABLE} (
            itr_year integer,
            source_archive_key varchar,
            source_run_id varchar,
...
def _ensure_statement_rows_usd_columns(connection: Any) -> None:
    qualified_table = f"{BRAZIL_CVM_DUCKDB_SCHEMA}.{ITR_STATEMENT_ROWS_TABLE}"
    connection.execute(
...
def _replace_year(*, connection: Any, year: str) -> None:
    for table_name in (
        ...
        connection.execute(
            f"delete from {BRAZIL_CVM_DUCKDB_SCHEMA}.{table_name} where itr_year = ?",
            [int(year)],
        )
...
def _load_documents(
    *,
    ...
    connection.execute(
        f"""
        insert into {BRAZIL_CVM_DUCKDB_SCHEMA}.{ITR_DOCUMENTS_TABLE}
        select
            'BR',
            ?,
            ?,
            ...
            source_row_number,
            ?
        from _brazil_fin_cvm_itr_member
        """,
        [
            SOURCE_SLUG,
            source_run_id,
...
def _load_statement_rows(
    *,
    connection: Any,
    csv_path: Path,
    member: BrazilCvmItrStatementMember,
    year: str,
    source_archive_key: str,
    source_run_id: str,
    resolved_at: datetime,
    ...
    columns = {
        row[0]
        for row in connection.execute("describe _brazil_fin_cvm_itr_member").fetchall()
    }
    period_start_expr = (
    ...
    insert_columns = ", ".join(ITR_STATEMENT_ROWS_COLUMNS)
    connection.execute(
        f"""
        insert into {BRAZIL_CVM_DUCKDB_SCHEMA}.{ITR_STATEMENT_ROWS_TABLE}
        ({insert_columns})
        select
            'BR',
            ?,
...
def _load_capital_composition(
    *,
    ...
    connection.execute(
        f"""
        insert into {BRAZIL_CVM_DUCKDB_SCHEMA}.{ITR_CAPITAL_COMPOSITION_TABLE}
        select
            'BR',
            ?,
            ?,
...
def _load_auditor_reports(
    *,
    ...
    report_type_expr = (
        ...
        if "TP_RELAT_AUD" in columns
        else "coalesce(TP_RELAT_ESP, '')"
        if "TP_RELAT_ESP" in columns
        else "''"
    )
    connection.execute(
        f"""
        insert into {BRAZIL_CVM_DUCKDB_SCHEMA}.{ITR_AUDITOR_REPORTS_TABLE}
        select
            'BR',
            ?,
            ?,
...
def _read_member_to_temp_table_with_encoding(
    *,
    ...
    connection.execute(
        f"""
        create or replace temporary table _brazil_fin_cvm_itr_member as
        select
...
def _year_counts(*, connection: Any, year: str) -> dict[str, int]:
    return {
        "document_row_count": _count_year(connection, ITR_DOCUMENTS_TABLE, year),
        "statement_row_count": _count_year(connection, ITR_STATEMENT_ROWS_TABLE, year),
...
def _count_year(connection: Any, table_name: str, year: str) -> int:
    return int(
        connection.execute(
            f"select count(*) from {BRAZIL_CVM_DUCKDB_SCHEMA}.{table_name} where itr_year = ?",
            [int(year)],
        ).fetchone()[0]
    )
...
def _record_parse_run(
    *,
    ...
    connection.execute(
        f"delete from {BRAZIL_CVM_DUCKDB_SCHEMA}.{ITR_PARSE_RUNS_TABLE} where itr_year = ?",
        [int(year)],
    )
    connection.execute(
        f"""
        insert into {BRAZIL_CVM_DUCKDB_SCHEMA}.{ITR_PARSE_RUNS_TABLE}
        values (?, ?, ?, ?, ?, ?, ?, ?)
        """,
        [
            int(year),
