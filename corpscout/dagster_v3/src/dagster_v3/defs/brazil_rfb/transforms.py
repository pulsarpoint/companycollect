...
def build_brazil_rfb_companies_and_establishments(
    *,
    ...
    source_run_id: str,
    empresas_database_path: str | Path,
    estabelecimentos_database_path: str | Path,
    simples_database_path: str | Path,
    ...
    with attached_read_only_database(
        ...
        connection.execute(
            ...
            select
                ...
                now() as resolved_at
            from {empresas_raw} emp
            left join picked p on p.cnpj_basico = emp.cnpj_basico
            left join {naturezas_raw} n on n.code = emp.natureza_juridica
