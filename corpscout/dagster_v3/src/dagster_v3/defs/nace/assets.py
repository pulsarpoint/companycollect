...
from dagster_v3.defs.clickhouse.resolved import replace_duckdb_tables_in_clickhouse
...
def nace_categories_duckdb(context: AssetExecutionContext) -> dg.MaterializeResult:
    counts = normalize_nace_categories_duckdb(duckdb_path=NACE_DUCKDB_PATH)
    context.log.info(
...
def normalize_nace_categories_duckdb(*, duckdb_path: str | Path) -> dict[str, int]:
...
def export_nace_categories_clickhouse(
    *,
    ...
    with duckdb.connect(str(duckdb_path), read_only=True) as connection:
        _validate_nace_duckdb_schema(connection)

    with clickhouse.get_connection() as client:
        counts = replace_duckdb_tables_in_clickhouse(
