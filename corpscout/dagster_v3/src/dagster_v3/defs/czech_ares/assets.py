...
def czech_ares_res_raw_duckdb(context: AssetExecutionContext) -> dg.MaterializeResult:
    context.log.info(
        "Loading Czech RES: url=%s, duckdb_path=%s", tables.RES_DATA_URL, CZECH_ARES_DUCKDB_PATH
    )
    rows = resources.load_czech_ares_res(
        database_path=CZECH_ARES_DUCKDB_PATH, log=context.log.info
    )
...
def czech_ares_companies_duckdb(context: AssetExecutionContext) -> dg.MaterializeResult:
    counts = resources.build_czech_ares_companies(
        database_path=CZECH_ARES_DUCKDB_PATH, source_run_id=context.run_id, log=context.log.info
    )
...
def czech_ares_industries_duckdb(context: AssetExecutionContext) -> dg.MaterializeResult:
