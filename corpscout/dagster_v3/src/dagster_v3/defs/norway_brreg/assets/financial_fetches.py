...
@dg.asset(
    ...
    description=(
        "Fetches Norway Brreg annual-account outcomes for the normalized snapshot "
        "no_companies parquet and stores raw plus aggregate parquet diagnostics."
    ),
)
def norway_brreg_financial_fetches_snapshot_parquet(
    ...
    context.log.info("Starting Norway Brreg snapshot financial parquet fetches")
    no_companies = norway_brreg_entity_storage.read_normalized_snapshot_table(
        ENTITY_NORMALIZED_TABLE_NO_COMPANIES
    )
    ...
    context.log.info(
        "Prepared Norway Brreg snapshot financial candidates: candidate_count=%d",
        len(candidates),
    )
    ...
    output_key = norway_brreg_financial_storage.write_snapshot_fetches(aggregate_frame)
    status_counts = _status_counts(rows)
    context.log.info(
        "Completed Norway Brreg snapshot financial parquet fetches: "
        "candidate_count=%d reused_count=%d fetched_count=%d row_count=%d "
        "status_counts=%s s3_key=%s",
        len(candidates),
        reused_count,
        fetched_count,
