...
**Architecture:** Attach a `MonthlyPartitionsDefinition` to the parse multi-asset only (its neighbors stay unpartitioned). For partition month `M`, the asset: (1) filters the S3 XML catalog to docs whose `registration_date` ∈ `M`; (2) **skips** docs whose `xml_object_key` is already present in the statement table; (3) parses the remaining docs with Arelle and **appends** to the two tables. Re-running a month skips everything already loaded → idempotent + resumable. Memory is bounded to one month's docs.

**Why this is correct (skip by unique S3 path):** every document has a unique S3 object key `companies/{business_id}/{financial_date}.xml` — one object per company per period — and the statement table already records `xml_object_key`. So "is this doc processed?" = "is this `xml_object_key` already in `fi_prh_xbrl_statement_documents`?" — no content hash, no catalog change, no per-doc delete. (We do **not** use the catalog's `xml_sha256` for skip: the download writes `xml_sha256=""` for *reused* documents — `assets.py:360` — and the catalog merge keeps that empty value, so it is unreliable. The unique object key sidesteps this entirely.) **Trade-off:** a corrected filing that overwrites the same object key won't be auto-re-parsed (same as the download's existence-based skip); a future `reparse_existing` flag covers that rare case. **No schema change needed.**

**Scope:** R1 only (the parse checkpoint). Rides along on the same asset: `pool="finland_ytj_duckdb"` (the R2 single-writer fix for this asset, free here since we're editing it). Out of scope: dbt migration of the SQL transforms (R4), incremental report list (R3), per-doc streaming below the month grain (future refinement if a peak month OOMs → `MultiPartitionsDefinition` month × hash-bucket), and a `reparse_existing` flag for corrected filings.
