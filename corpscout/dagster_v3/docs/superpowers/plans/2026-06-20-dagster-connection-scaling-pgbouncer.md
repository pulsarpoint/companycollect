...
- **Pool mode: start with `transaction` (best multiplexing), keep `session` as the fallback.** Dagster does use session-level features (LISTEN/NOTIFY event watcher, occasional advisory locks). In practice the bulk storage ops work under `transaction` pooling; if you observe DB-related hangs or misbehaving live-log tailing that trace to pooling (NOT to unrelated causes — see next bullet), switch the service to `POOL_MODE: session` and bump `DEFAULT_POOL_SIZE` (it then holds a server conn per *active* client connection). Either mode achieves the core goal: **total server connections are capped at `default_pool_size`.**
- **Don't misattribute a startup hang to PgBouncer.** `dg dev` hanging at "Launching Dagster services…" is frequently a **stale dbt manifest file lock** held by an orphaned code-server process (`*/dbt/target/manifest.concurrent-update-lock`), unrelated to the DB. Before blaming pooling: kill leftover `dagster api grpc` processes and `find src/dagster_v3/defs -name '*.concurrent-update-lock' -delete`.
- **Size `default_pool_size` for the realistic peak** (persistent webserver + daemon + code server + concurrent run subprocesses). Start at 50. Keep the client-side `--db-pool-max-overflow` modest (~30) since PgBouncer now bounds the server side.
