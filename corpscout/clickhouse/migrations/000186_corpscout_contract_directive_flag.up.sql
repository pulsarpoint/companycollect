...
-- Record whether the EU procurement directives govern a contract, because it is
-- what separates "no value exists anywhere" from "the value is in TED and we
-- have not loaded it".
--
-- Sweden's UHM publishes no contract value at all -- the register has 44 columns
-- and none is monetary, confirmed against the live file. Directive-governed
-- procurement is also published in TED, which does carry a per-winner awarded
-- amount. So a Swedish contract with no value is one of two very different
-- things, and until now both rendered as the same empty cell.
--
-- Each source answers in its own terms: UHM publishes the flag, a TED notice
-- implies it by existing, and Hilma implies it when it cross-refers to TED.
-- Where a source says nothing the value is empty, not "no" -- absence of a
-- reference is not evidence of being below the threshold.
ALTER TABLE corpscout.se_uhm_procurement_awards
    ADD COLUMN IF NOT EXISTS directive_governed LowCardinality(String)
    AFTER advertising_database;
