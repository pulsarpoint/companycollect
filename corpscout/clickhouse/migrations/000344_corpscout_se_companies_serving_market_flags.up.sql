...
-- Adds three presence flags to corpscout.se_companies_serving (owner 2026-08-28):
--   is_publicly_traded        an ESEF filing exists for the company's LEI (esef_filings via
--                             company_identifier -- listed on an EU regulated market)
--   has_government_contracts  exact-matched winner rows in corpscout.se_government_contracts
--                             (UHM + TED), the same view the public contracts pages read
--   has_job_ads               corpscout.company_job_history rows for SE (Platsbanken ads,
--                             open now or in the past -- table empty until that pipeline runs)
--
-- 000320-STYLE STAGED SWAP (live readers), with one addition learned from 000338's apply: the
-- old view's own 15-minute refresh is STOPPED first, so the _next build cannot OOM-collide
-- with a concurrent old-view refresh on the server's memory cap (two ~20 GiB builds against
-- 27.31 GiB killed 000338's first attempt). The stopped machinery leaves with the _retired
-- name via the follow-up drop migration -- no DROP here.
--
...
FROM (
  SELECT
    ...
    toUInt8(i.company_id IN (SELECT ci.company_id FROM corpscout.company_identifier AS ci WHERE ci.issuer_scheme = 'lei' AND ci.country_code = 'SE' AND ci.is_current = 1 AND ci.issuer_id IN (SELECT upperUTF8(trimBoth(f.lei)) FROM corpscout.esef_filings AS f))) AS is_publicly_traded,
