...
-- Folds the registered-activity translation and status-reason label into
-- corpscout.se_companies_serving, absorbed VERBATIM from corpscout.se_companies_translated
-- (migration 000252's rendering) together with the spine fields its one executable reader
-- (company_serving's company_description_current_build dbt model) needs: activity_description,
-- activity_description_en, status_reason, status_reason_label_en, bolagsverket_source_record_uid,
-- updated_from_raw_at. The legal-form label join is NOT carried over -- se_company_info already
-- ships label_en/label_sv (000306). The translated view itself is dropped by the follow-up
-- migration once the dbt model is repointed and deployed.
--
-- STAGED SWAP, NOT A PLAIN CREATE -- 000320's pattern, for 000320's reason: unlike 000335,
-- corpscout.se_companies_serving now HAS live readers (the backoffice companies and geocoding
-- lists read it on every page). Building under _next, waiting for its first refresh, and
-- swapping both names in ONE atomic RENAME means a reader sees either the old view or the fully
-- populated new one -- never UNKNOWN_TABLE and never an empty view. The old view keeps its
-- machinery under the _retired name (still refreshing) and is dropped by the follow-up
-- migration -- no DROP here (house rule).
--
-- THE SELECT BELOW IS NOT HAND-WRITTEN AND MUST NOT BE HAND-EDITED. It is the exact rendering
-- of companies_current.build_se_companies_serving_sql() -- the drift pin in dagster_v3
-- tests/test_se_companies_serving_mv.py now points at THIS migration.
