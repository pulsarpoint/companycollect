...
-- Give a contract an identity of its own, separate from the companies that won
-- it, so it can be listed and opened as a thing in itself.
--
-- contract_id was per winner: it carried winner_ordinal, and for UHM the
-- company id. A contract with three winners was three unrelated ids, so there
-- was no way to ask "show me this contract" or to gather its winners. It is now
-- the contract within its source -- source, notice, lot -- and the winner is
-- described by the columns that already describe one.
--
-- dedup_key becomes contract_key for the same reason. It was buyer, date and
-- title hashed WITH the company, which identified "this company's involvement
-- in this contract". Without the company it identifies the contract, which is
-- what matches the same procurement across a national register and TED. The
-- summary combines it with company_id and gets the old meaning back.
--
-- winner_name comes along because a contract page has to name its winners,
-- including the ones with no company id to link to.
