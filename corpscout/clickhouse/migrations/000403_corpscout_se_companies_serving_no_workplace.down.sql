...
-- Undoes 000403: the serving view goes back to the render 000398 deployed, in which a
-- workplace-only address row counts in the addresses array and in address_count and can win
-- the primary pick. Same in-place recipe as the up file -- stop, repoint, start -- because
-- only the view's own SELECT changes, and for the same reason there is no SYSTEM WAIT VIEW
-- to sit through. Nothing else has to come back: no row ever left
-- corpscout.se_company_address, this migration only stopped the view from publishing some of
-- them. Running this restores the database only -- the deployed dagster code
-- (companies_current.py) must be rolled back with it, or the next render taken from the
-- builder re-applies the exclusion.
--
-- THE SELECT BELOW IS 000398's, COPIED VERBATIM rather than hand-written, and dagster_v3
