...
-- Count current source-reported domains, including unverified Brave candidates.
-- Live reviewer rejections and withdrawn domains do not count.
-- Generated from companies_current.build_se_companies_serving_sql.
-- Keep existing serving rows until the next refresh completes.
-- If interrupted after STOP, verify the installed query and START the view
-- before repairing the migration ledger.
...
FROM (
  SELECT
    ...
    toUInt8(i.company_id IN (SELECT domains.company_id
      FROM corpscout.company_domains_resolved AS domains
