...
-- Faithful recreate of the eodhd-era serving render (000347's embedded SELECT, preserved
-- verbatim) under the _retired name, refresh STOPPED as 000347 left it. Roll forward is the
-- sane path -- this exists so the ledger can walk backwards. The guard DROP makes the
-- recreate idempotent on a half-applied down.
