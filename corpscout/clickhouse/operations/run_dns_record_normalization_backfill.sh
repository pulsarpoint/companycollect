...
start_bucket="${DNS_BACKFILL_START_BUCKET:-0}"
end_bucket="${DNS_BACKFILL_END_BUCKET:-15}"
phase="${DNS_BACKFILL_PHASE:-both}"
...
if [[ "${phase}" != "both" && "${phase}" != "records" && "${phase}" != "sightings" ]]; then
    echo "PHASE must be one of: both, records, sightings" >&2
    exit 2
fi
...
docker "${docker_args[@]}" "${client_image}" "${client_args[@]}" --query "
    SELECT throwIf(
        count() != 4,
        'DNS record normalization tables are missing; apply ClickHouse migration 155 first'
    )
    FROM system.tables
...
for (( bucket = start_bucket; bucket <= end_bucket; bucket++ )); do
    echo "DNS record normalization backfill bucket ${bucket}/15 (${phase})"

    if [[ "${phase}" == "both" || "${phase}" == "records" ]]; then
        docker "${docker_args[@]}" "${client_image}" "${client_args[@]}" \
            --param_bucket="${bucket}" \
            --queries-file=/operations/dns_records_backfill_bucket.sql
    fi

    if [[ "${phase}" == "both" || "${phase}" == "sightings" ]]; then
        docker "${docker_args[@]}" "${client_image}" "${client_args[@]}" \
            --param_bucket="${bucket}" \
            --queries-file=/operations/dns_record_sightings_backfill_bucket.sql
    fi

    echo "DNS record normalization backfill bucket ${bucket}/15 complete"
done
