...
func main() {
	...
	if err != nil {
		slog.Error("parse SEC EDGAR import command", "error", err)
		os.Exit(2)
	}

	if err := run(context.Background(), cfg); err != nil {
		slog.Error("run SEC EDGAR import command",
			"command", cfg.command,
...
func parseArgs(args []string) (cliConfig, error) {
	...
	flags.StringVar(&cfg.dataDir, "data-dir", "", "SEC EDGAR data directory")
...
func run(ctx context.Context, cli cliConfig) error {
	...
	cfg := secedgar.ConfigFromEnv()
	if cli.dataDir != "" {
		cfg.DataDir = cli.dataDir
	}
	source := secedgar.NewSource(cfg)

	switch cli.command {
	case "download":
		result, err := source.Download(ctx, countryimport.DownloadOptions{DataDir: cli.dataDir})
		if err != nil {
			return errors.Wrap(err, "download SEC EDGAR snapshot")
		}
		slog.Info("downloaded SEC EDGAR snapshot",
	...
	case "process":
		...
		if err != nil {
			return errors.Wrap(err, "process SEC EDGAR snapshot")
		}
		slog.Info("processed SEC EDGAR snapshot",
			"snapshot_path", result.SnapshotPath,
			"records", result.RecordsProcessed,
			"chunks", result.ChunksProcessed,
		)
	case "run":
		downloadResult, err := source.Download(ctx, countryimport.DownloadOptions{DataDir: cli.dataDir})
		if err != nil {
			return errors.Wrap(err, "download SEC EDGAR snapshot")
		}
		...
		slog.Info("ran SEC EDGAR import",
