...
func main() {
	...
	if err != nil {
		slog.Error("parse IRS EO BMF import command", "error", err)
		os.Exit(2)
	}

	if err := run(context.Background(), cfg); err != nil {
		slog.Error("run IRS EO BMF import command",
			"command", cfg.command,
...
func parseArgs(args []string) (cliConfig, error) {
	...
	flags.StringVar(&cfg.dataDir, "data-dir", "", "IRS EO BMF data directory")
	flags.IntVar(&cfg.maxFiles, "max-files", 0, "maximum regional CSV files to download (0 = all)")
...
func run(ctx context.Context, cli cliConfig) error {
	...
	cfg := irseobmf.ConfigFromEnv()
	if cli.dataDir != "" {
		cfg.DataDir = cli.dataDir
	}
	source := irseobmf.NewSource(cfg)

	switch cli.command {
	case "download":
		result, err := source.Download(ctx, countryimport.DownloadOptions{
			DataDir:  cli.dataDir,
			MaxPages: cli.maxFiles,
		})
		if err != nil {
			return errors.Wrap(err, "download IRS EO BMF snapshot")
		}
		slog.Info("downloaded IRS EO BMF snapshot",
			"snapshot_path", result.SnapshotPath,
			"files", result.PagesDownloaded,
	...
	case "process":
		...
		if err != nil {
			return errors.Wrap(err, "process IRS EO BMF snapshot")
		}
		slog.Info("processed IRS EO BMF snapshot",
			"snapshot_path", result.SnapshotPath,
	...
	case "run":
		...
		slog.Info("ran IRS EO BMF import",
			"snapshot_path", downloadResult.SnapshotPath,
			"files", downloadResult.PagesDownloaded,
