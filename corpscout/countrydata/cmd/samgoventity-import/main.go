...
func main() {
	...
	if err != nil {
		slog.Error("parse SAM.gov entity import command", "error", err)
		os.Exit(2)
	}

	if err := run(context.Background(), cfg); err != nil {
		slog.Error("run SAM.gov entity import command",
			"command", cfg.command,
...
func parseArgs(args []string) (cliConfig, error) {
	...
	flags.StringVar(&cfg.dataDir, "data-dir", "", "SAM.gov entity data directory")
...
func run(ctx context.Context, cli cliConfig) error {
	...
	cfg := samgoventity.ConfigFromEnv()
	if cli.dataDir != "" {
		cfg.DataDir = cli.dataDir
	}
	source := samgoventity.NewSource(cfg)
	...
	case "download":
		...
		if err != nil {
			return errors.Wrap(err, "download SAM.gov entity snapshot")
		}
		slog.Info("downloaded SAM.gov entity snapshot",
	...
	case "process":
		...
		if err != nil {
			return errors.Wrap(err, "process SAM.gov entity snapshot")
		}
		slog.Info("processed SAM.gov entity snapshot",
			"snapshot_path", result.SnapshotPath,
	...
	case "run":
		...
		slog.Info("ran SAM.gov entity import",
