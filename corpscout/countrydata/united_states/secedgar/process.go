...
func (s *Source) Process(ctx context.Context, opts countryimport.ProcessOptions) (countryimport.ProcessResult, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return result, countryimport.WrapSourceError(
				...
				snapshotPath,
				0,
				errors.Wrap(err, "open SEC snapshot"),
			)
		}
		return result, countryimport.WrapSourceError(
	...
	records := make([]SecTickerRecord, 0, chunkSize)
	flush := func() error {
		if len(records) == 0 {
			return nil
		}
		...
		if err != nil {
			return countryimport.WrapSourceError(
				...
				snapshotPath,
				0,
				errors.Wrap(err, "store SEC records"),
			)
		}
	...
	for {
		...
		if err := json.Unmarshal(scanner.Bytes(), &record); err != nil {
			result.DecodeErrors++
			slog.WarnContext(ctx, "decode SEC EDGAR snapshot line",
				"source", SourceSlug,
				"line", lineNumber,
				"error", err,
			)
	...
	if err := scanner.Err(); err != nil {
		return result, countryimport.WrapSourceError(
			...
			snapshotPath,
			0,
			errors.Wrap(err, "scan SEC snapshot"),
		)
	}
	if err := ctx.Err(); err != nil {
		return result, processContextError(err, snapshotPath)
...
func processContextError(err error, snapshotPath string) error {
	return countryimport.WrapSourceError(
		...
		snapshotPath,
		0,
		errors.Wrap(err, "process SEC snapshot"),
	)
}
...
func latestSnapshotPath(snapshotsDir string) (string, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return "", countryimport.WrapSourceError(
				...
				snapshotsDir,
				0,
				errors.Wrap(err, "find SEC snapshot"),
			)
		}
		return "", countryimport.WrapSourceError(
			countryimport.ErrorKindFileIO,
	...
	for _, entry := range entries {
		...
		if err != nil {
			return "", countryimport.WrapSourceError(
				...
				0,
				errors.Wrap(err, "stat SEC snapshot"),
			)
		}
	...
	if latestPath == "" {
		return "", countryimport.WrapSourceError(
			...
			snapshotsDir,
			0,
			errors.New("no SEC snapshot found"),
		)
	}
