...
func (s *Source) Process(ctx context.Context, opts countryimport.ProcessOptions) (countryimport.ProcessResult, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return result, countryimport.WrapSourceError(
				...
				snapshotPath,
				0,
				errors.Wrap(err, "open EO BMF snapshot"),
			)
		}
		return result, countryimport.WrapSourceError(
	...
	flush := func() error {
		...
		if err != nil {
			return countryimport.WrapSourceError(
				...
				snapshotPath,
				0,
				errors.Wrap(err, "store EO BMF records"),
			)
		}
	...
	if err := scanner.Err(); err != nil {
		return result, countryimport.WrapSourceError(
			...
			snapshotPath,
			0,
			errors.Wrap(err, "scan EO BMF snapshot"),
		)
	}
	if err := ctx.Err(); err != nil {
		return result, processContextError(err, snapshotPath)
...
func processContextError(err error, snapshotPath string) error {
	return countryimport.WrapSourceError(
		...
		snapshotPath,
		0,
		errors.Wrap(err, "process EO BMF snapshot"),
	)
}
...
func latestSnapshotPath(snapshotsDir string) (string, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return "", countryimport.WrapSourceError(
				...
				snapshotsDir,
				0,
				errors.Wrap(err, "find EO BMF snapshot"),
			)
		}
		return "", countryimport.WrapSourceError(
			countryimport.ErrorKindFileIO,
	...
	for _, entry := range entries {
		...
		if err != nil {
			return "", countryimport.WrapSourceError(
				...
				0,
				errors.Wrap(err, "stat EO BMF snapshot"),
			)
		}
	...
	if latestPath == "" {
		return "", countryimport.WrapSourceError(
			...
			snapshotsDir,
			0,
			errors.New("no EO BMF snapshot found"),
		)
	}
