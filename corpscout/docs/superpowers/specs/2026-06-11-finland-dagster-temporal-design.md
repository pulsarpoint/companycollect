...
|---|---|---|---|
| `finland_prhytj/raw_snapshot` | materializable | Pull company snapshot + 7 code lists from PRH API, stream to `source-finland-prhytj` bucket under `runs/{run_id}/`, write `manifest.json` | Schedule (weekly cron, configurable) |
...
| `finland_prhytj/code_lists` | materializable | Import the 7 code-list TSVs (REK, REK_KDI, VIRANOM, TLAJI, YRMU, STATUS3, KIELI) into `fi_prhytj_code_lists` | eager on `raw_snapshot` |
...
`run_id` for bucket keys is the UTC start timestamp plus a short unique
suffix (`20260611T120000Z-1a2b3c4d` — the Dagster run id for Dagster-produced
runs, the Temporal workflow run id for Temporal-produced runs), so two
launches or retries in the same second can never overwrite each other's
prefix while prefixes stay human-sortable by time. It is recorded as
materialization metadata together with object keys, sha256s, byte sizes, and
record counts.
...
```json
{
  "run_id": "20260611T120000Z-1a2b3c4d",
  "source": "finland_prh_xbrl",
  "workflow_id": "xbrl-download-20260611T120000Z",
  "artifacts": [
    {
      "key": "statements_manifest",
      "object_key": "runs/20260611T120000Z-1a2b3c4d/statements.ndjson",
      "content_sha256": "...",
...
source-finland-prhytj/
  ...
  runs/{run_id}/codelists/REK.en.tsv     (7 code lists)
