...
def iter_companies(base_url: str, page_size: int = spec.PAGE_SIZE) -> Iterator[dict]:
    ...
    while True:
        response = session.get(base_url, params={"page": page_number}, timeout=_TIMEOUT_SECONDS)
...
def fetch_code_list(base_url: str, code: str, lang: str) -> bytes:
    ...
    response = requests.get(url, params={"code": code, "lang": lang}, timeout=_TIMEOUT_SECONDS)
    response.raise_for_status()
