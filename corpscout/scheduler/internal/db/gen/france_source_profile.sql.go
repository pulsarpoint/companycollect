...
// versions:
//   sqlc v1.30.0
// source: france_source_profile.sql
...
const deleteFranceSourceIndustriesForLegalUnits = `-- name: DeleteFranceSourceIndustriesForLegalUnits :exec
...
WHERE source_company.id = industry.company_id
  ...
  AND source_company.raw_legal_unit_id = ANY($1::uuid[])
...
func (q *Queries) DeleteFranceSourceIndustriesForLegalUnits(ctx context.Context, rawLegalUnitIds []uuid.UUID) error {
	_, err := q.db.Exec(ctx, deleteFranceSourceIndustriesForLegalUnits, rawLegalUnitIds)
	return err
}

const demoteFranceSourceHeadquartersForLegalUnits = `-- name: DemoteFranceSourceHeadquartersForLegalUnits :exec
...
func (q *Queries) DemoteFranceSourceHeadquartersForLegalUnits(ctx context.Context, rawLegalUnitIds []uuid.UUID) error {
	_, err := q.db.Exec(ctx, demoteFranceSourceHeadquartersForLegalUnits, rawLegalUnitIds)
	return err
}

const insertFranceSourceIndustries = `-- name: InsertFranceSourceIndustries :one
WITH company_industries AS (
  ...
  FROM france_source.companies source_company
  WHERE source_company.raw_legal_unit_id = ANY($1::uuid[])
    AND source_company.row_status = 'active'
...
inserted AS (
  ...
  SELECT
    ...
    now()
  FROM (
    SELECT company_id, establishment_id, raw_legal_unit_id, raw_establishment_id, source_field, position, source_code, source_nomenclature, naf25_code, is_primary, raw_industry_payload FROM company_industries
    UNION ALL
    SELECT company_id, establishment_id, raw_legal_unit_id, raw_establishment_id, source_field, position, source_code, source_nomenclature, naf25_code, is_primary, raw_industry_payload FROM establishment_industries
  ) industry_rows
  RETURNING id
)
...
type InsertFranceSourceIndustriesParams struct {
	RawLegalUnitIds []uuid.UUID `json:"raw_legal_unit_ids"`
	Trigger         string      `json:"trigger"`
}

func (q *Queries) InsertFranceSourceIndustries(ctx context.Context, arg InsertFranceSourceIndustriesParams) (int32, error) {
	row := q.db.QueryRow(ctx, insertFranceSourceIndustries, arg.RawLegalUnitIds, arg.Trigger)
	var rows_inserted int32
	err := row.Scan(&rows_inserted)
	return rows_inserted, err
}

const selectFranceSourceProfileLegalUnitIDs = `-- name: SelectFranceSourceProfileLegalUnitIDs :many
SELECT rr.id
  ...
  AND (
    ...
    OR EXISTS (
      ...
      WHERE selected_establishment.siren = rr.siren
        AND (
          selected_establishment.id::text = ANY($1::text[])
          OR selected_establishment.siret = ANY($1::text[])
        )
    )
  )
  AND (
    $2::text IS NULL
    OR rr.siren ILIKE '%' || $2::text || '%'
    OR rr.legal_name ILIKE '%' || $2::text || '%'
    OR rr.usual_name_1 ILIKE '%' || $2::text || '%'
    OR rr.primary_activity_code ILIKE '%' || $2::text || '%'
    OR EXISTS (
      SELECT 1
      ...
      WHERE searched_establishment.siren = rr.siren
        ...
        AND (
          searched_establishment.siret ILIKE '%' || $2::text || '%'
          OR searched_establishment.trade_name_1 ILIKE '%' || $2::text || '%'
          OR searched_establishment.usual_name ILIKE '%' || $2::text || '%'
          OR searched_establishment.city_label ILIKE '%' || $2::text || '%'
        )
    )
  )
  AND (
...
ORDER BY rr.siren
LIMIT GREATEST($3::integer, 1)
`

type SelectFranceSourceProfileLegalUnitIDsParams struct {
	Ids   []string `json:"ids"`
	Query *string  `json:"query"`
	Limit int32    `json:"limit"`
}

func (q *Queries) SelectFranceSourceProfileLegalUnitIDs(ctx context.Context, arg SelectFranceSourceProfileLegalUnitIDsParams) ([]uuid.UUID, error) {
	rows, err := q.db.Query(ctx, selectFranceSourceProfileLegalUnitIDs, arg.Ids, arg.Query, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const supersedeFranceSourceCompaniesForLegalUnits = `-- name: SupersedeFranceSourceCompaniesForLegalUnits :exec
...
func (q *Queries) SupersedeFranceSourceCompaniesForLegalUnits(ctx context.Context, rawLegalUnitIds []uuid.UUID) error {
	_, err := q.db.Exec(ctx, supersedeFranceSourceCompaniesForLegalUnits, rawLegalUnitIds)
	return err
}

const supersedeFranceSourceEstablishmentsForLegalUnits = `-- name: SupersedeFranceSourceEstablishmentsForLegalUnits :exec
UPDATE france_source.establishments source_establishment
SET
  row_status = 'superseded',
  superseded_at = now(),
...
func (q *Queries) SupersedeFranceSourceEstablishmentsForLegalUnits(ctx context.Context, rawLegalUnitIds []uuid.UUID) error {
	_, err := q.db.Exec(ctx, supersedeFranceSourceEstablishmentsForLegalUnits, rawLegalUnitIds)
	return err
}

const upsertFranceSourceAddresses = `-- name: UpsertFranceSourceAddresses :one
WITH base AS (
  SELECT
    source_establishment.company_id,
    source_establishment.id AS establishment_id,
...
shaped AS (
  SELECT
    company_id, establishment_id, raw_establishment_id, address_type, address_rank, complement, street_number, street_number_suffix, last_street_number, last_street_number_suffix, street_type, street_name, postal_code, city, foreign_city, special_distribution, commune_code, cedex_code, cedex_label, foreign_country_code, foreign_country, address_identifier, lambert_x, lambert_y,
...
type UpsertFranceSourceAddressesParams struct {
	RawLegalUnitIds []uuid.UUID `json:"raw_legal_unit_ids"`
	Trigger         string      `json:"trigger"`
}

func (q *Queries) UpsertFranceSourceAddresses(ctx context.Context, arg UpsertFranceSourceAddressesParams) (int32, error) {
	row := q.db.QueryRow(ctx, upsertFranceSourceAddresses, arg.RawLegalUnitIds, arg.Trigger)
	var rows_upserted int32
	err := row.Scan(&rows_upserted)
	return rows_upserted, err
}

const upsertFranceSourceCompanies = `-- name: UpsertFranceSourceCompanies :one
WITH selected AS (
  SELECT
    rr.id, rr.source_file_id, rr.source_native_id, rr.siren, rr.diffusion_status, rr.is_purged, rr.created_date, rr.acronym, rr.gender, rr.first_name_1, rr.first_name_2, rr.first_name_3, rr.first_name_4, rr.usual_first_name, rr.pseudonym, rr.association_identifier, rr.employee_band, rr.employee_year, rr.source_updated_at, rr.period_count, rr.company_category, rr.company_category_year, rr.period_started_at, rr.administrative_status, rr.birth_name, rr.usage_name, rr.legal_name, rr.usual_name_1, rr.usual_name_2, rr.usual_name_3, rr.legal_form_code, rr.primary_activity_code, rr.primary_activity_nomenclature, rr.headquarters_nic, rr.social_solidarity, rr.mission_company, rr.is_employer, rr.primary_activity_naf25_code, rr.raw_payload, rr.payload_hash, rr.is_current, rr.first_seen_at, rr.last_seen_at, rr.metadata,
    COALESCE(
...
type UpsertFranceSourceCompaniesParams struct {
	RawLegalUnitIds []uuid.UUID `json:"raw_legal_unit_ids"`
	Trigger         string      `json:"trigger"`
}

func (q *Queries) UpsertFranceSourceCompanies(ctx context.Context, arg UpsertFranceSourceCompaniesParams) (int32, error) {
	row := q.db.QueryRow(ctx, upsertFranceSourceCompanies, arg.RawLegalUnitIds, arg.Trigger)
	var rows_upserted int32
	err := row.Scan(&rows_upserted)
	return rows_upserted, err
}

const upsertFranceSourceEstablishments = `-- name: UpsertFranceSourceEstablishments :one
WITH selected AS (
  SELECT
    raw_establishment.id, raw_establishment.source_file_id, raw_establishment.source_native_id, raw_establishment.siren, raw_establishment.nic, raw_establishment.siret, raw_establishment.diffusion_status, raw_establishment.created_date, raw_establishment.employee_band, raw_establishment.employee_year, raw_establishment.crafts_registry_activity, raw_establishment.source_updated_at, raw_establishment.is_headquarters, raw_establishment.period_count, raw_establishment.address_complement, raw_establishment.street_number, raw_establishment.street_number_suffix, raw_establishment.last_street_number, raw_establishment.last_street_number_suffix, raw_establishment.street_type, raw_establishment.street_label, raw_establishment.postal_code, raw_establishment.city_label, raw_establishment.foreign_city_label, raw_establishment.special_distribution, raw_establishment.commune_code, raw_establishment.cedex_code, raw_establishment.cedex_label, raw_establishment.foreign_country_code, raw_establishment.foreign_country_label, raw_establishment.address_identifier, raw_establishment.lambert_x, raw_establishment.lambert_y, raw_establishment.address_complement_2, raw_establishment.street_number_2, raw_establishment.street_number_suffix_2, raw_establishment.street_type_2, raw_establishment.street_label_2, raw_establishment.postal_code_2, raw_establishment.city_label_2, raw_establishment.foreign_city_label_2, raw_establishment.special_distribution_2, raw_establishment.commune_code_2, raw_establishment.cedex_code_2, raw_establishment.cedex_label_2, raw_establishment.foreign_country_code_2, raw_establishment.foreign_country_label_2, raw_establishment.period_started_at, raw_establishment.administrative_status, raw_establishment.trade_name_1, raw_establishment.trade_name_2, raw_establishment.trade_name_3, raw_establishment.usual_name, raw_establishment.primary_activity_code, raw_establishment.primary_activity_nomenclature, raw_establishment.is_employer, raw_establishment.primary_activity_naf25_code, raw_establishment.raw_payload, raw_establishment.payload_hash, raw_establishment.is_current, raw_establishment.first_seen_at, raw_establishment.last_seen_at, raw_establishment.metadata,
    source_company.id AS company_id,
    COALESCE(
...
type UpsertFranceSourceEstablishmentsParams struct {
	RawLegalUnitIds []uuid.UUID `json:"raw_legal_unit_ids"`
	Trigger         string      `json:"trigger"`
}

func (q *Queries) UpsertFranceSourceEstablishments(ctx context.Context, arg UpsertFranceSourceEstablishmentsParams) (int32, error) {
	row := q.db.QueryRow(ctx, upsertFranceSourceEstablishments, arg.RawLegalUnitIds, arg.Trigger)
	var rows_upserted int32
	err := row.Scan(&rows_upserted)
	return rows_upserted, err
}
