...
func TestBrregWorkflowTaskSelectionUsesArtifactsAsCompletionTruth(t *testing.T) {
	...
	require.Contains(t, selectionQuery, "COALESCE(cardinality(sqlc.arg('selected_ids')::text[]), 0) > 0 AS selected_by_id")
	require.Contains(t, selectionQuery, "artifact_needed")
	require.Contains(t, selectionQuery, "sqlc.arg('task_type')::text = 'translate'")
	require.Contains(t, selectionQuery, "ri.translation_status IN ('not_started', 'failed')")
	require.Contains(t, selectionQuery, "ri.domain_status IN ('not_started', 'failed')")
	require.Contains(t, selectionQuery, "ri.financial_status IN ('not_started', 'failed')")
	require.Contains(t, selectionQuery, "selected_by_id")
	require.Contains(t, selectionQuery, "task_status IN ('failed_retryable', 'failed_terminal')")
	require.NotContains(t, selectionQuery, "ts.status IN ('succeeded', 'skipped')")

	require.Contains(t, claimQuery, "selected_by_id")
	require.Contains(t, claimQuery, "jsonb_typeof(s.selection_definition->'ids') = 'array'")
	require.Contains(t, claimQuery, "needs_artifact")
	require.Contains(t, claimQuery, "AND srr.needs_artifact")
	require.Contains(t, claimQuery, "srr.selected_by_id")
	require.Contains(t, claimQuery, "ts.status IN ('failed_retryable', 'failed_terminal')")
