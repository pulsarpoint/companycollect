...
> in-domain A, AAAA, and CNAME observations. The split scanners write only the shared observation
> history, and migration 129 removes the old physical registry after that writer cutover was
> verified in production. `commoncrawl_domains` remains the root-domain input to the DNS scanner.
