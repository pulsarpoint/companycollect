...
Install the package with `python -m pip install -e .` from
`corpscout/services/crawler_service`. Configure these values in the process environment or one explicitly
selected env file:
...
```sh
crawler-service-crawl https://example.com --env-file /path/to/crawler.env \
  --technology-catalog data/technology-catalog.json \
  --output-dir data/example > example.json

crawler-service-submit data/example/result.json \
...
uvx ty check --python ../.venv src/crawler_service
```
