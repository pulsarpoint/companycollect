...
address is DHCP-assigned) as the system unit `crawler-service.service`. The
...
Deploy that service first, then configure `crawler_service_browser_api_url` and
`crawler_service_browser_api_token`. This playbook no longer installs Chromium,
...
slots, configured with `crawler_service_concurrency`, matching the two browsers
on this deployment. Up to `crawler_service_max_pending` further requests wait in
the service's local queue; beyond that REST submissions are rejected.
Manual retries have two worker slots by default,
configured with `crawler_service_manual_concurrency`; browser capacity is external.
...
```yaml
crawler_service_s3_bucket: crawls
crawler_service_s3_endpoint_url: http://rustfs:9000
crawler_service_s3_access_key: replace-me
crawler_service_s3_secret_key: replace-me
```

The default object prefix is `company-crawls`. Override
`crawler_service_s3_prefix` in deployment variables if needed. The environment
...
| `/etc/systemd/system/crawler-service.service` | System unit, enabled at boot |
...
```sh
ssh graovic@crawler 'sudo systemctl status crawler-service --no-pager'
ssh graovic@crawler 'sudo journalctl -u crawler-service -n 50 --no-pager'
...
```sh
sudo -u crawler-service env \
  HOME=/var/lib/crawler-service \
  CLOAKBROWSER_CACHE_DIR=/var/cache/company-research/cloakbrowser \
  CLOAKBROWSER_AUTO_UPDATE=false \
  /opt/companycollect/corpscout/company_research/current/.venv/bin/crawler-service-crawl \
...
Version 0.42.0 uses the `crawler_service` Python package, `crawler-service` REST
command and `crawler-service.service` systemd unit. The controller variables use
the `crawler_service_` prefix; update existing ignored `secrets.yml` keys from
`company_research_` before deployment. Credential values stay the same.

The Linux account remains `company-research`, and application releases, credentials,
state and cache retain their existing paths shown above. This deliberately preserves
SQLite history, queued jobs, absolute artifact paths and filesystem permissions.
The HTTP API, port 8080, browser service connection, S3 object keys and result schemas
are unchanged. Local source and editable installations now live in `services/crawler_service`;
run `uv sync --all-extras --group test --reinstall` after moving an existing checkout.
