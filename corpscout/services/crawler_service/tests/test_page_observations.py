...
from crawler_service.clickhouse_results import result_row
from crawler_service.crawl import crawl_company
from crawler_service.identifiers import identifier_validation
...
class ObservationTests(unittest.TestCase):
    ...
    def test_nonfinite_json_and_readability_failure_do_not_destroy_capture(self):
        with patch(
            "crawler_service.page_observations.trafilatura.extract",
    ...
    async def test_full_rendered_observations_survive_both_retention_modes_and_import(
        ...
        for retain in [True, False]:
            with self.subTest(retain=retain), TemporaryDirectory() as temporary:
                ...
                with (
                    patch("crawler_service.crawl.open_browser", browser),
                    patch(
                        "crawler_service.page_observations.collect_technology_observations",
