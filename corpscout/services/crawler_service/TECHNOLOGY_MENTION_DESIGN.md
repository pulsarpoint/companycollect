...
The output boundary is a complete JSON object containing all research results,
source sections, raw mentions and later classifications. Storage belongs to the
caller; the former RustFS/S3 integration is removed.
See [the output contract](TECHNOLOGY_MENTION_STORAGE.md). Parsing and storing
...
proposals. Suggestions are retained in the returned JSON; downstream database ingestion
...
`crawler_service/statements.py:PageStatement` and the existing statement
