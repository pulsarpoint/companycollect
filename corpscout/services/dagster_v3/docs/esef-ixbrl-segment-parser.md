...
Three independent `esef_filings` assets project that result into
`esef_document_people`, `esef_document_business_items`, and
`esef_document_group_relationships`. Each projection reads the
company-information table directly and can be materialized or retried separately.
Company descriptions are read directly from `esef_document_company_information`
by the company-serving build, alongside the typed registry and Wikidata sources.
