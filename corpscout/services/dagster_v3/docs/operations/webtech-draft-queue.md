...
Legacy tasks with NULL queue_scope are never adopted as open drafts. The retired Common Crawl Webtech partition workflow is no longer registered. Load Common Crawl domains using `webtech_scan_input` with `source_relation: corpscout.commoncrawl_domain_graph_signals` and the desired crawl/rank filters, then start `webtech_scan_results`. See [input selection](webtech-scan-input.md). Historical results and scanner reports remain available.
