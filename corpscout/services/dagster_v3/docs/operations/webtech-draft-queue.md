...
The asset then loops: pull up to `batch_size` remaining input rows, submit them to the scanner as one envelope, and publish results as they arrive, until no remaining rows are returned. "Remaining" is this execution's frozen inputs minus inputs that already have a result for this execution's `crawl_id` (`webtech-<execution_id>`), minus inputs whose freshest result within `[freshness_cutoff, started_at]` for the current detector version was a success (skipped as fresh; `force_rescan` disables this second filter). Bounding staleness checks to `<= started_at` means results this execution itself writes can never change what counts as fresh on a resume. Envelopes are a transport detail only: the pages travel inline in the `POST /v1/scans` body (`crawl_id`, `detector_version` and the `candidates` list), the scanner derives the scan ID from the crawl ID, detector version, sorted input IDs and its own settings, and nothing about the envelope is persisted anywhere — no object-store manifest and no per-envelope or per-bucket bookkeeping in Postgres or ClickHouse.
...
Results are published in micro-batches, not one insert per result: a `ResultBuffer` flushes every 500 results or 5 seconds (ClickHouse creates one part per insert, so batching avoids part storms). A publish failure while an envelope is still polling keeps its rows buffered for the next attempt; only a failure at end-of-envelope flush fails the run. After each envelope, if any of its inputs still has no published result (a progress event was missed), the run polls the completed scan once from event 0 — which replays every stored result reference of the scan — and publishes those inputs, which is idempotent; any input still missing fails the run. The scanner stores only per-page reports (`scans/detector_version=…/crawl_id=…/pages/input_id=…/report.json`); there are no candidate or completion manifests to clean up.
...
Legacy tasks with NULL queue_scope are never adopted as open drafts. The retired Common Crawl Webtech partition workflow is no longer registered. Load Common Crawl domains using `webtech_scan_input` with `source_relation: corpscout.commoncrawl_domain_graph_ranks` and the one explicit graph_release and the desired rank filters, then start `webtech_scan_results`. See [input selection](webtech-scan-input.md). Historical results and scanner reports remain available.
