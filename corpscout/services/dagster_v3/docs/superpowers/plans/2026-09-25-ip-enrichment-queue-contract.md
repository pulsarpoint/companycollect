...
First check that the install directory is writable by the Dagster service user (the install writes a temporary file next to the installed ones and renames it): `ssh dagster 'DIR="$(sudo grep "^MAXMIND_DATABASE_DIRECTORY=" /opt/companycollect/corpscout/dagster_v3/.env | cut -d= -f2)"; ls -ld "$DIR" "$DIR"/*.mmdb; systemctl show -p User dagster-code-server 2>/dev/null'` (find the unit's user if the unit name differs); if the directory belongs to another user, `chown` it to the service user once. Then the owner opens Admin → Settings → GeoLite2 (`/admin/settings/geolite2`) and uploads `GeoLite2-City_20260925.tar.gz` and `GeoLite2-ASN_20260925.tar.gz` in one submission. The page stores them unchanged in bucket `geolite2` (`uploads/<uuid>/<name>`, created on first use) and launches `geolite2_install_job`; it refreshes until the run ends.
Expected: the install run is `SUCCESS`; its `geolite2_databases` materialization shows `installed = ["City", "ASN"]`, `city_build`/`asn_build` on 2026-09-2x, `fresh = true`, and the page shows both builds with their age in days (not red). Then execute `geolite2_freshness_job`: `geolite2_databases_fresh` passes. On the host, `ls -l "$DIR"` shows the two files with the Dagster service user as owner and no `.GeoLite2-*.tmp` leftovers. A refused install (older build, wrong edition, bad archive) replaces nothing; the reason is in the run's failure. This must be done before Step 10; Step 9 may run before it (its metadata then reports the July builds).
