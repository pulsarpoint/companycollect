...
8. Geocode block on candidates: `foreign` candidates get `geocode_status = 'foreign'`, everything else empty/None, `geocode_policy = ''`, `geocode_reference = ''`, `geocoded_at = None`, and `needs_geocode()` is False. Every other candidate leaves the block empty and `needs_geocode()` True; the batch fills it via `with_geocode(outcome)`: `latitude`, `longitude`, `geocode_status = outcome.match_status`, `geocode_method = outcome.coordinate_method if outcome.geocode_provider == GEOCODE_FALLBACK_PROVIDER else outcome.match_method` (amended 2026-09-07: gate on the serving overlay's own provider tag, not `match_status == GEOCODE_FALLBACK_STATUS` -- the resolver itself can emit `matched_area` for a multi-candidate area match, and that outcome's `match_method` still names its own strategy), `geocode_confidence = outcome.match_confidence`, `geocode_precision = outcome.geocode_precision`, `geocode_policy = outcome.policy_version`, `geocode_reference = outcome.reference_md5`, `geocoded_at = outcome.matched_at`.
