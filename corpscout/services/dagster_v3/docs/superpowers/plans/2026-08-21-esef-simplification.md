...
| 7 | `esef_document_company_information` | One LLM asset: anti-join on `(source_document_id, model_name, prompt_version)`, evidence from S3 artifact, DeepSeek call, result rows + one content-addressed response artifact in S3. The 4 normalized observation tables are NOT written here — they remain the `company_source_records` group's projection (see Phase 4 contract point 4) | the existing 6-line LEFT JOIN skip |
...
| `esef_document_observations_clickhouse`, `esef_company_source_records_clickhouse` (group `company_source_records`) | KEPT AS-IS — cross-source provenance layer (identity scheme + orphan guards + live dbt/backoffice consumers); only their `deps` re-point to targets 5/7 |
...
4. The LLM asset writes ONLY `esef_document_company_information` to CH. The 4 observation tables (`company_description_observations`, `esef_document_people`, `esef_document_business_items`, `esef_document_group_relationships`) stay where they are: `company_source_records/esef_document_observations_clickhouse` projects them from the information table, because their `source_record_uid`s are minted by that group's identity scheme (`identity.py`) and its orphan anti-join guard requires the source-records spine populated first. Only its `deps` re-point to the collapsed LLM asset. Rename or split `reprocess_existing_without_model` into `reselect_processed` + `force_model_call`.
