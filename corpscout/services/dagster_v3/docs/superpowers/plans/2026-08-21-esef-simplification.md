...
| 7 | `esef_document_company_information` | One LLM asset: anti-join on `(source_document_id, model_name, prompt_version)`, evidence from S3 artifact, DeepSeek call, result + 4 normalized observation tables + one content-addressed response artifact in S3 | the existing 6-line LEFT JOIN skip |
...
| `esef_document_company_information_duckdb` / `_clickhouse` / `esef_document_observations_clickhouse` | → target 7 |
...
4. Write results + the 4 observation tables straight to CH (all four have live dbt/backoffice consumers — the tables stay). Rename or split `reprocess_existing_without_model` into `reselect_processed` + `force_model_call`.
