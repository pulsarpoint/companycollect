...
- Extractors (spec 5.2, 5.3) insert into `SE_COMPANY_FIELD_CANDIDATE` in `SE_COMPANY_FIELD_CANDIDATE_COLUMNS` order; `value_json` of json fields must carry the members the projection reads: `count`, `as_of` (ISO date) for `employee_count`; `amount`, `currency`, `amount_usd`, `fiscal_year` as JSON numbers / strings for `latest_revenue`; `language` for `description`; `compare_key` everywhere agreement should not fall back to `lowerUTF8(trim(value))`.
