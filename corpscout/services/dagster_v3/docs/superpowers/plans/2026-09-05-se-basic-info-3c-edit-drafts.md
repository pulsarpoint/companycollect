...
- Validation per field kind (client parser and server agree): `legal_name` non-empty text ≤ 500; `legal_form_code` one of the SCB codes offered (`^[0-9]{2}$` and present in the options the loader supplied); `status` `active` or `inactive`; `incorporation_date` `YYYY-MM-DD`, between 1900-01-01 and today (the Date32 floor; the plan first said 1800-01-01, corrected at the final review); `lei` `^[A-Z0-9]{20}$` (upper-cased); `wikidata_id` `^Q[0-9]+$`; `description` and `description_sv` non-empty text ≤ 8000, plain text; `description` also carries `language` ∈ {`en`, `sv`}. Values are trimmed; an empty value is refused ("Value cannot be empty.") since Discard is the way to clear.
...
- `validateSeBasicInfoValue(field, value, language, options): { ok: true; value: string; language: string } | { ok: false; error: string }` where `options = { legalFormCodes: readonly string[]; today: string }` and the rules are the Global Constraints' (messages: "Value cannot be empty.", "Value is longer than N characters.", "Legal form must be one of the SCB codes.", "Status must be active or inactive.", "Date must be YYYY-MM-DD.", "Date must be between 1900-01-01 and today (the Date32 floor; the plan first said 1800-01-01, corrected at the final review).", "LEI must be 20 letters or digits.", "Wikidata id must be Q followed by digits.", "Language must be en or sv."). `language` is only used for `description` (returned `''` otherwise).
