...
active from SCB code 1. The economic-activity flag is its own entity field since slice 6 (`2026-09-08-se-basic-info-6-economic-activity-design.md`): `economic_activity`, values active / never / ceased, '' when unknown like `status`, migration 000394. A precedence change is not a per-company change, so it takes a full re-fold
...
Amended 2026-09-05 (owner decision, slice 3b): "Use this" no longer copies a value into the reviewer row; it inserts a company rule `(company_id, field, source, 10000, decided_by 'backoffice', note)` into the precedence table, so the chosen source's *current* value flows through every later fold exactly as an automatic pick does, and a change in that source is a history row like any other. The panel orders sources by effective precedence, marks the ruled source "preferred by reviewer" with its note, and "fold pending" also considers a rule newer than the fold. Amended 2026-09-05 (slice 3c, owner decision, Edit with drafts): an "Edit value" button per field opens a right-side sheet with a form built for the field's kind (single-line text for the legal name, a select over the SCB legal-form codes with both labels, active/inactive for the status, a date input (accepted between 1900-01-01, the Date32 floor, and today; ClickHouse would silently saturate an earlier date), format-checked inputs for LEI and Wikidata id, a multi-line text area for the descriptions with a language select for the English-facing one; plain text, no markdown). Saving writes a new version of the company's `reviewer_draft` row with that field set (and the description's language); nothing folds until the reviewer activates it. The panel shows the draft as a "Draft" row with Activate and Discard per field: Activate inserts a new `reviewer` row version with the field copied from the draft (and a draft version with it cleared), Discard clears it in the draft. "Reset to default" for a field also clears the active reviewer value, so one button returns the field to the pipeline whether a rule or a typed value held it. The left card flags fields with a pending draft. Amended later the same day (owner request): the per-row "Release" is replaced by a field-level "Reset to default" button in the panel header, shown when the company has any active rule for the selected field; confirming it inserts a `removed = 1` version for every active rule of that field in one write (note "reset to default", plus the reviewer's), so the global precedence decides again at the next fold. The left card flags such fields with a "custom order" badge. The reviewer row's value columns are reserved for Edit.
