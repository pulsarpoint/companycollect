...
def test_esef_select_keeps_swedish_issuers_with_a_description() -> None:
    ...
    assert (
        "info.country_iso2 = 'SE'" in sql
        and "match(info.company_id, '^([0-9]{10}|[0-9]{12})$')" in sql
    )
...
def test_esef_asset_depends_on_the_document_information_asset() -> None:
    ...
    asset = (
        load_defs()
        .get_repository_def()
        .asset_graph.get(dg.AssetKey("se_company_info_esef_clickhouse"))
    )
...
class _FakeClient:
    ...
    def __init__(
        self, *, existing_tables: set[str], answers: list[list[tuple]]
    ) -> None:
...
def test_esef_asset_publishes_new_versions_only_via_left_anti_join() -> None:
    client = _FakeClient(
        ...
        answers=[
            ...
            [(1,)],
            [(11,)],
        ],  # staged/invalid, existing, anti-join, total
    )
