...
def test_cgu_clickhouse_export_coalesces_cnep_null_strings(tmp_path: Path) -> None:
    db_path = tmp_path / "cgu.duckdb"
    with duckdb.connect(str(db_path)) as connection:
        connection.execute(f"create schema {parsing.BRAZIL_CGU_DUCKDB_SCHEMA}")
        connection.execute(
            f"""
            create table {parsing.BRAZIL_CGU_DUCKDB_SCHEMA}.{tables.CNEP_COMPANY_SANCTIONS_TABLE}
            as select
                'BR'::varchar as country_iso2,
                'brazil_cgu'::varchar as source_slug,
                'run-1'::varchar as source_run_id,
                'record-1'::varchar as source_record_id,
                '2026-07-06'::varchar as snapshot_date,
                'cnep'::varchar as source_dataset,
                'https://example.test/cnep.zip'::varchar as source_url,
                'archive.zip'::varchar as source_archive_key,
                '20260706_CNEP.csv'::varchar as source_file_name,
                2::ubigint as source_row_number,
                'CNEP'::varchar as registry,
                '90923'::varchar as sanction_id,
                '13221906000188'::varchar as cnpj,
                '13221906'::varchar as cnpj_basico,
                'J'::varchar as person_type,
                null::varchar as sanctioned_name,
                null::varchar as sanctioning_agency_reported_name,
                null::varchar as receita_legal_name,
                null::varchar as receita_trade_name,
                null::varchar as process_number,
                null::varchar as sanction_category,
                null::decimal(38, 6) as fine_amount_brl,
                null::varchar as sanction_start_date,
                null::varchar as sanction_end_date,
                null::varchar as publication_date,
                null::varchar as publication,
                null::varchar as publication_detail,
                null::varchar as final_judgment_date,
                null::varchar as sanction_scope,
                null::varchar as sanctioning_agency,
                null::varchar as sanctioning_agency_state,
                null::varchar as sanctioning_agency_sphere,
                null::varchar as legal_basis,
                null::varchar as source_information_date,
                null::varchar as information_origin,
    ...
    assert rows == 1
    _, inserted_rows = fake_client.inserts[0]
    columns = tables.CGU_TABLES[tables.CNEP_COMPANY_SANCTIONS_TABLE].columns
    assert inserted_rows[0][columns.index("snapshot_date")] == date(2026, 7, 6)
    assert inserted_rows[0][columns.index("fine_amount_brl")] is None
    assert inserted_rows[0][columns.index("sanction_start_date")] is None
    assert inserted_rows[0][columns.index("sanctioned_name")] == ""
    assert inserted_rows[0][columns.index("publication")] == ""
    assert inserted_rows[0][columns.index("information_origin")] == ""
