...
- `DenmarkCvrSearchResource` launches CloakBrowser once per partition, opens the public result page, and performs sequential credentialed JSON POST requests in that page.
...
- If the gateway returns an empty page before the first page's advertised total is exhausted, the asset fails rather than silently accepting a truncated capture.
...
- Large searches can return hundreds of thousands of advertised results. Pagination uses the first response's total as the completion contract so later inconsistent empty pages fail visibly.
