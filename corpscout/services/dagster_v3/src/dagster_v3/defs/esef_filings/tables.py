...
# Shadow tables for the partition-isolated parsing migration. These remain
# separate from the serving tables until the backfill and parity gates in
# esef-parsing-migration-plan.md have passed.
ESEF_SOURCE_DOCUMENTS_V2_TABLE = "esef_source_documents_v2"
ESEF_FACTS_V2_TABLE = "esef_facts_v2"
ESEF_DOCUMENT_CONTACT_CANDIDATES_V2_TABLE = (
    "esef_document_contact_candidates_v2"
)
ESEF_DOCUMENT_CONCEPT_LABELS_V2_TABLE = "esef_document_concept_labels_v2"
ESEF_FACT_DISCLOSURES_V2_TABLE = "esef_fact_disclosures_v2"
...
QUALIFIED_ESEF_SOURCE_DOCUMENTS_V2_TABLE = (
    f"{ESEF_DATABASE}.{ESEF_SOURCE_DOCUMENTS_V2_TABLE}"
)
QUALIFIED_ESEF_FACTS_V2_TABLE = f"{ESEF_DATABASE}.{ESEF_FACTS_V2_TABLE}"
QUALIFIED_ESEF_DOCUMENT_CONTACT_CANDIDATES_V2_TABLE = (
    f"{ESEF_DATABASE}.{ESEF_DOCUMENT_CONTACT_CANDIDATES_V2_TABLE}"
)
QUALIFIED_ESEF_DOCUMENT_CONCEPT_LABELS_V2_TABLE = (
    f"{ESEF_DATABASE}.{ESEF_DOCUMENT_CONCEPT_LABELS_V2_TABLE}"
)
QUALIFIED_ESEF_FACT_DISCLOSURES_V2_TABLE = (
    f"{ESEF_DATABASE}.{ESEF_FACT_DISCLOSURES_V2_TABLE}"
)
...
# Every v2 table carries its Dagster partition explicitly. The ClickHouse
# migration partitions on this column, allowing one completed weekly DuckDB
# file to replace exactly one ClickHouse partition atomically.
ESEF_SOURCE_DOCUMENTS_V2_EXPORT_COLUMNS = (
    *ESEF_SOURCE_DOCUMENTS_EXPORT_COLUMNS,
    "processed_week",
)
ESEF_FACTS_V2_EXPORT_COLUMNS = (*ESEF_FACTS_EXPORT_COLUMNS, "processed_week")
ESEF_DOCUMENT_CONTACT_CANDIDATES_V2_EXPORT_COLUMNS = (
    *ESEF_DOCUMENT_CONTACT_CANDIDATES_EXPORT_COLUMNS,
    "processed_week",
)
ESEF_DOCUMENT_CONCEPT_LABELS_V2_EXPORT_COLUMNS = (
    *ESEF_DOCUMENT_CONCEPT_LABELS_EXPORT_COLUMNS,
    "processed_week",
)
ESEF_FACT_DISCLOSURES_V2_EXPORT_COLUMNS = (
    *ESEF_FACT_DISCLOSURES_EXPORT_COLUMNS,
    "processed_week",
)
