...
def esef_document_observation_sql() -> tuple[str, ...]:
    description_uid = clickhouse_observation_uid_sql(
    ...
    candidate_uid = clickhouse_observation_uid_sql(
        source_record_uid_expression="info.source_record_uid",
        observation_kind="esef_typed_candidate",
        natural_key_expression=(
            "concat(info.prompt_version, ':', info.model_name, ':', item_kind, ':', "
            "item_json)"
        ),
    )
    extracted_at = (
...
SELECT
    {candidate_uid.replace("item_kind", "'person'")},
    info.source_record_uid,
...
WHERE info.extraction_status IN ('complete', 'enriched', 'reused')
  ...
  AND JSONExtractString(item_json, 'role') != ''""",
        f"""INSERT INTO {tables.QUALIFIED_ESEF_DOCUMENT_BUSINESS_ITEMS_TABLE}
...
WHERE JSONExtractString(item_json, 'name') != ''""",
        f"""INSERT INTO {tables.QUALIFIED_ESEF_DOCUMENT_GROUP_RELATIONSHIPS_TABLE}
({", ".join(tables.ESEF_DOCUMENT_GROUP_RELATIONSHIP_COLUMNS)})
SELECT
    {candidate_uid.replace("item_kind", "'group_relationship'")},
    info.source_record_uid,
    info.source_document_id,
    info.country_iso2,
    info.company_id,
