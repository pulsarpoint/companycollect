...
def replace_osm_address_points_from_relation(
    *,
    ...
    """Build the address index with two source scans instead of retaining all OSM rows."""
    ...
    try:
        ...
        connection.execute(
            """
            create or replace temporary table _osm_address_way_refs as
            select
                address.osm_id,
                ref.node_id,
                ref.ref_order,
        ...
        connection.execute(
            f"""
            create or replace temporary table _osm_address_way_nodes as
            select
                ref.osm_id,
                ref.node_id,
                ref.ref_order,
            ...
            inner join _osm_address_way_refs ref
        ...
        connection.execute(
            """
            create or replace temporary table _osm_address_way_points as
        ...
        connection.execute(
            ...
            with candidates as (
                ...
                from _osm_address_objects address
                inner join _osm_address_way_points point
                    on point.osm_id = address.osm_id
                where address.osm_type = 'way'
            ), address_fields as (
                select
