...
`company-research-crawl URL --instructions "Get current job listings and full job descriptions" --output-dir PATH`.
...
Use `company-research-crawl URL --output-dir PATH` for automatic discovery, or add
repeatable `--page URL` options to fetch an exact list without LLM calls. Then run
`company-research-pages --crawl PATH` to analyze the saved folder independently.
...
caller; `company-research-pages` emits one JSON object on stdout and progress on
...
Development checks, from `corpscout/services/company_research`:
