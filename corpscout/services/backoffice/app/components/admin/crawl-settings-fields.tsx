 ...
 */
export function CrawlSettingsFields({type, idPrefix}: {type: DomainCrawlType; idPrefix: string}) {
  ...
  return <FieldGroup className="grid grid-cols-1 gap-4 sm:grid-cols-2">
    ...
    <Field><FieldLabel htmlFor={id("api")}>Crawl model API</FieldLabel><NativeSelect id={id("api")} name="api" defaultValue="deepseek" required><NativeSelectOption value="deepseek">DeepSeek</NativeSelectOption><NativeSelectOption value="openrouter">OpenRouter</NativeSelectOption></NativeSelect></Field>
    <Field><FieldLabel htmlFor={id("model")}>Crawl model</FieldLabel><Input id={id("model")} name="model" defaultValue="deepseek-flash" maxLength={200} required /></Field>
