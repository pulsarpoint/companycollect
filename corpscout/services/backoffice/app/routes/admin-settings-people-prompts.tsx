...
import type { Route } from "./+types/admin-settings-people-prompts";
import { Button } from "~/components/ui/button";
import { Input } from "~/components/ui/input";
import { Label } from "~/components/ui/label";
import { Textarea } from "~/components/ui/textarea";
import { listPeoplePrompts, savePeoplePrompt, PeoplePromptValidationError } from "~/lib/people-prompts.server";

export async function loader({ request }: Route.LoaderArgs) {
  const url = new URL(request.url);
  const prompts = listPeoplePrompts();
...
export async function action({ request }: Route.ActionArgs) {
  ...
  try {
    const id = savePeoplePrompt(values);
    return redirect(`/admin/settings/people-prompts?edit=${encodeURIComponent(id)}&saved=yes`);
  } catch (error) {
    if (error instanceof PeoplePromptValidationError) return { error: error.message, values };
    throw error;
  }
}

export function meta() { return [{ title: "People prompts | CompanyCollect" }]; }

export default function PeoplePrompts({ loaderData, actionData }: Route.ComponentProps) {
  ...
  return <main className="flex flex-col gap-6 p-6">
    <div><h1 className="text-2xl font-semibold">People prompts</h1>
      <p className="mt-1 text-sm text-muted-foreground">Saved instructions for matching people across Swedish registers. Choose a prompt and an LLM for the Full processing action.</p></div>
    <div className="grid gap-8 lg:grid-cols-[280px_minmax(0,1fr)]">
      <nav aria-label="Saved People prompts" className="flex flex-col gap-2">
        ...
        <Button variant="outline" render={<Link to="/admin/settings/people-prompts" />}>New prompt</Button>
      </nav>
      <Form method="post" key={`${editing?.promptId ?? "new"}:${editing?.revision ?? 0}`} className="flex max-w-4xl flex-col gap-4">
        ...
        <div className="space-y-2"><Label htmlFor="system-prompt">Matching instructions</Label>
          <p className="text-xs text-muted-foreground">The model also receives candidate JSON with IDs such as c0 and c1. Ask it to return {`{"pairs":[{"a":"c0","b":"c1","confidence":0.9,"reason":"…"}]}`} or {`{"pairs":[]}`}.</p>
