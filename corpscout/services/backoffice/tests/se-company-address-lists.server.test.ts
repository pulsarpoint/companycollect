...
describe("se_company_address_correction list SQL shape", () => {
  it("reads the ledger, joins the published rows aggregated per company, and names the decided address key", () => {
    ...
    expect(ADDRESS_SCOPED_PUBLISHED_JOIN_SQL).toContain("GROUP BY company_id");
  });

  /**
   * The published side is aggregated per company because a company has several
   * address rows. Applied ids come from ALL of them (a reject's id lives on the
   * tombstone it wrote), while the key and hash sets are the LIVE rows only --
   * exactly the split se-company-address.server.ts makes for one company.
   */
  it("aggregates applied ids over every row and the key/hash sets over live rows only", () => {
    expect(ADDRESS_SCOPED_PUBLISHED_JOIN_SQL).toContain(
  ...
  it("computes the four statuses, with the reject-with-no-row branch inside applied", () => {
    ...
    // company does not publish, so the absence of the key IS the applied signal.
    expect(ADDRESS_CORRECTION_STATUS_EXPR).toContain("c.correction_kind = 'reject_address'");
    expect(ADDRESS_CORRECTION_STATUS_EXPR).toContain("NOT has(p.live_address_keys,");
    expect(ADDRESS_CORRECTION_STATUS_EXPR).toContain("{zeroHash:String}");
    expect(ADDRESS_CORRECTION_STATUS_EXPR).toContain("NOT has(p.live_evidence_hashes,");
  });
