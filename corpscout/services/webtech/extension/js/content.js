...
const WEBTECH_PAGE_TOKEN_ATTRIBUTE = 'data-webtech-page-token'
const WEBTECH_PAGE_TOKEN = crypto.randomUUID()

document.documentElement.setAttribute(
  WEBTECH_PAGE_TOKEN_ATTRIBUTE,
  WEBTECH_PAGE_TOKEN
)
...
function inject(src, id, message) {
  return new Promise((resolve) => {
    ...
    timeout = setTimeout(() => finish(), INJECT_TIMEOUT_MS)
...
const Content = {
  ...
  async init() {
    ...
    const stageTimingsMs = { content_started: 0 }
    let currentStage = 'content_started'
    const analysisDeadlineTimer = setTimeout(() => {
      stageTimingsMs.analysis_timed_out = Math.max(
        0,
        Math.round(performance.now() - startedAt)
      )
      void Content.driver('analysisTimedOut', [
        url,
        currentStage,
        stageTimingsMs,
      ])
    ...
    const enterStage = async (stage) => {
      currentStage = stage
      const elapsedMs = Math.max(0, Math.round(performance.now() - startedAt))

      stageTimingsMs[stage] = elapsedMs
      await Content.driver('analysisProgress', [url, stage, elapsedMs])
    }

    await enterStage(currentStage)

    if (await Content.driver('isDisabledDomain', url)) {
      await enterStage('finalizing')
      clearTimeout(analysisDeadlineTimer)
      await Content.driver('analysisComplete', [url, stageTimingsMs])
    ...
    try {
      await enterStage('initial_metadata')
      ...
      await enterStage('initial_dom_js')
      ...
      await enterStage('heavy_signals')
      ...
      await enterStage('delayed_wait')
      await new Promise((resolve) => setTimeout(resolve, 5000))

      await enterStage('delayed_dom_js')
      const js = await getJs(technologies)
      const dom = await getDom(technologies)

      await Promise.all([
        Content.driver('analyzeJs', [url, js]),
      ...
      clearTimeout(analysisDeadlineTimer)
      stageTimingsMs.analysis_failed = Math.max(
        0,
        Math.round(performance.now() - startedAt)
      )
      await Content.driver('analysisFailed', [
        url,
        currentStage,
        error instanceof Error ? error.message : String(error),
        stageTimingsMs,
      ])
      await Content.driver('error', error)
    }
  },
