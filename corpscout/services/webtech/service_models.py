...
CRAWL_ID_PATTERN = re.compile(r"webtech-[a-f0-9-]{36}")
...
class ScanRequest(BaseModel):
    """One envelope of a queue execution, sent inline by Dagster."""
    ...
    def validate_crawl_id(cls, value: str) -> str:
        if CRAWL_ID_PATTERN.fullmatch(value) is None or (
            str(UUID(value.removeprefix("webtech-"))) != value.removeprefix("webtech-")
        ):
            raise ValueError("crawl_id must be webtech-<execution uuid>")
        return value

    @model_validator(mode="after")
    def validate_candidates(self) -> Self:
        if len({candidate.task_id for candidate in self.candidates}) > 1:
            raise ValueError("scan request must contain inputs from one task")
        for candidate in self.candidates:
            try:
                canonical_task = str(UUID(candidate.task_id)) == candidate.task_id
            except ValueError:
